...
/** Hard caps to keep a malformed (or hostile) Java-serialised stream
 *  from exhausting the renderer. The legitimate BMP responses we've
 *  seen sit at depth ~30 and array length ~3000, so the headroom here
 *  is conservative. Hitting either throws — the SW handler turns it
 *  into an error toast rather than crashing the worker. */
const MAX_READ_DEPTH = 200;
...
export class JavaReader {
  ...
  private pos: number;
  private handles: any[] = [];
  /** Current nesting depth in readObject(). Incremented before each
   *  recursive read and checked against MAX_READ_DEPTH; decremented in
   *  a finally block so an exception unwinds the counter. */
  private depth = 0;
  // Block data mode
  ...
  /** Read any content element from the stream. Tracks recursion depth
   *  so a hostile stream of nested objects can't blow the JS stack. */
  readObject(): any {
    if (this.pos >= this.bytes.length) return null;
    if (this.depth >= MAX_READ_DEPTH) {
      throw new Error(`Java-serial: max object nesting (${MAX_READ_DEPTH}) exceeded at pos ${this.pos}`);
    }
    this.depth++;
    try {
  ...
  private readNewArray(): any {
    ...
    if (count < 0) {
      throw new Error(`Java-serial: negative array length ${count} at pos ${this.pos}`);
    }
    if (count > MAX_ARRAY_LENGTH) {
      throw new Error(`Java-serial: array length ${count} exceeds cap ${MAX_ARRAY_LENGTH} at pos ${this.pos}`);
    }
    const arr: any[] = [];
