...
When a profile uses a stored configuration login, CREV sends the username and password directly to that BMP server's legacy direct-login endpoint. BMP defines this as a GET request, so the credentials are URL query parameters and may be retained by BMP or reverse-proxy access logs. CREV does not log or retain that request URL. Use HTTPS and only enable this mode where the server operator protects or redacts access logs. This path requests an independent command ticket with browser cookies omitted; it does not sign the BMP website in as the stored account.
